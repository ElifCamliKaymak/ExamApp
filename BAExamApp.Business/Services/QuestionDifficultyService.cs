...
public class QuestionDifficultyService : IQuestionDifficultyService
{
    ...
    public async Task<IDataResult<List<QuestionDifficultyListDto>>> GetQuestionDifficultiesByQuestionTypeAsync(string examType)
    {
        if (examType == "2")
        {
            var classicQuestionDifficulties = await _questionDiffucultyRepository.GetAllAsync(x => x.TimeGiven.Minutes >= 3);

            return new SuccessDataResult<List<QuestionDifficultyListDto>>(_mapper.Map<List<QuestionDifficultyListDto>>(classicQuestionDifficulties), Messages.ListedSuccess);
        }

        var questionDifficulties = await _questionDiffucultyRepository.GetAllAsync(x => x.TimeGiven.Minutes < 3);
