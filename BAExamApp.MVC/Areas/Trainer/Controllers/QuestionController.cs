...
public class QuestionController : TrainerBaseController
{
    ...
    public async Task<IActionResult> QuestionList(State state)
    {
        ...
        var questionListResult = await _questionService.GetAllByStateAndTrainerIdAsync(UserIdentityId, state);
    ...
    public async Task<IActionResult> Update(Guid id)
    {
        ...
        ViewBag.ProductList = await GetProductsAsync();
        ViewBag.QuestionTypes = GetQuestionTypes();
        ViewBag.SubjectId = questionUpdateVM.SubjectId;
        ViewBag.SubtopicId = questionUpdateVM.SubtopicId;
        ViewBag.QuestionAnswersList = System.Text.Json.JsonSerializer.Serialize(questionUpdateVM.QuestionAnswers);
        ...
        if (ModelState.IsValid)
        {
            if (questionAnswersList.Count >= 1)
            {
                ...
                if (!questionRevisionUpdateFailed)
                {
                    ...
                    var questionResult = await _questionService.UpdateAsync(_mapper.Map<QuestionUpdateDto>(updateQuestionVM));
    ...
    public async Task<IActionResult> Details(Guid id)
    {
        ...
        return PartialView("_QuestionDetailsPartial", questionDetails);
    }
    ...
    [HttpGet]
    public async Task<SelectList> GetQuestionDifficulties(string questionTypeId)
    {
        return new SelectList((await _questionDifficultyManager.GetQuestionDifficultiesByQuestionTypeAsync(questionTypeId)).Data, "Id", "Name");
    }
    ...
    [HttpGet]
    public async Task<string> GetQuestionTimeByDifficultyId(string difficultyId)
    {
        return (await _questionDifficultyManager.GetDetailsByIdAsync(Guid.Parse(difficultyId))).Data.TimeGiven.ToString();
    }
