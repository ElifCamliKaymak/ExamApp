...
public class SubtopicController : AdminBaseController
{
    ...
    public async Task<IActionResult> Update(Guid id)
    {
        ...
        var getResult = await _subtopicService.GetSubtopicById(id);
        if (!getResult.IsSuccess)
        {
            NotifyErrorLocalized(getResult.Message);
            return RedirectToAction(nameof(Index));
        }
        var updateSubtopicResult = _mapper.Map<AdminSubtopicUpdateVM>(getResult.Data);
        return View(updateSubtopicResult);
    }
